class Sudoku
  ...
  def solve_one(cell_index)
      ...
      total_possibilities.map {|num|
        ...
        if occurrence_hash.select {|num, occurrence| occurrence == 3}.keys.length == 1
         @board_arr[cell_index/9][cell_index%9] = occurrence_hash.select {|num, occurrence| occurrence == 3}.keys.join('')
        end
  end
...
# testing = File.readlines('sudoku_puzzles.txt').to_s
# game2 = Sudoku.new(testing)
p game.to_s
