...
class Sudoku
  ...
  def solve
    ...
    #     @board_array = @board_array.each_with_index { |item, index| (index == e) ? possibilities[0] : "-"}
    #     self.solve
    #     break
    ...
    until solved?
      @board_array.each_with_index do |value, index|
        if value == "-"
          possibilities = full_number - (@row_hash[index] + @col_hash[index] + @box_hash[index]).uniq
          if possibilities.size == 1
            @board_array[index] = possibilities[0]
          end
        end
      end
      return false if @board_array.join == @board
    end
    return @board.join
  end
