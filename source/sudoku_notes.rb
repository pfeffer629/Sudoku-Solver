class Sudoku
  ...
  def initialize(board_string)
    ## Can make a string instead, perhaps? # Can also give it 
    @board_arr = board_string.split("") #Refactor to get the split arr on one line
  ...
  def is_incomplete? # Is complete
    return false if not @board_arr.flatten.include?("-")
  end

  def find_blank # Find next blank - a little more descriptive - next blank coords, e.g.
    # Is this the right place for this whole loop? Maybe not
  ...
  def find_blank
    [@board_string.index("-")/9, @board_string.index("-")%9] #can do this with an index in the same way you could a string.
    # @board_arr.flatten.index("-").divmod(9) #Gives the divide and the modolo of that number. Allows you to basically take a structure of the same height and width. Index gives the position of that item you are looking for. 
  end

  def check_row #Separate methods for defining and checking row?
  ...
  def identify_the_4_remaining_in_box
    ...
    @row_return = @blank_pos[0]/3 
    @col_return = @blank_pos[1]/3 
    @box_checks = []
    2.times do |r|
      2.times do |c|
